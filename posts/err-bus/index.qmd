...
The metadata block. The useful fields are `line_name` (`"L02"`), `line_color` (`"#e81e63"`, the route's own pink, which I reuse for every marker and line on the map), and the two human-readable direction labels, `aller_direction` (`"Ahibous(31)"`) and `retour_direction` (`"Omrane (1)"`).
